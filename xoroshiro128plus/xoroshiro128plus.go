...
// Ported to Go by Amy Snow in 2020.
...
*/
var jumpConst [2]uint64 = [2]uint64{0xdf900294d8f554a5, 0x170865df4b3201fc}
var longJumpConst [2]uint64 = [2]uint64{0xd2a98b26625eee7b, 0xdddf9b1090aa7ac1}

//from http://vigna.di.unimi.it/xorshift/xoroshiro128plus.c, ported to Go
func rotl(x uint64, k uint) uint64 {
	return (x << k) | (x >> (64 - k))
}

func (gen *Generator) next() (result uint64) {
	s0, s1 := gen.rngS0, gen.rngS1
	result = gen.rngS0 + gen.rngS1
	s1 ^= s0
	gen.rngS0 = rotl(s0, 24) ^ s1 ^ (s1 << 16) // a, b
	gen.rngS1 = rotl(s1, 37)                   // c
	return result
}
